...
# v0.3   fixed the PRS method PRS = GT * OR /N_SNP

# v0.3.1 output the SNP_id each bait used
...
all_snp_list_array = np.array(all_snp_list)
...
f_out.write('\t'.join(['IP\tbait\tint\tn_SNP\tsum_gene_len']) + '\n' )
...
for bait in bait_list:
    ...
    for ip in ip_list:
        ...
        for i in int_type:  # y , n
            ...
            use_snp_id_list = all_snp_list_array[snp_used]  # used SNP ID

            out_line = [ip, bait, i, str(snp_num), str(gene_len)] + use_snp_id_list.tolist()
